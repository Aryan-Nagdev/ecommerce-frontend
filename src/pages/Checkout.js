...
const Checkout = () => {
  ...
  // FIX: Send clean order items to backend
  ...
  const handlePayment = async () => {
    try {
      ...
      if (data.message === "Order placed") {
        try {
          clearCart(); // Prevent crash if cart state corrupt
        } catch (err) {
          console.warn("Cart clearing warning (ignored):", err);
        }
      ...
      // Backend returned wrong text — shouldn't happen now
        ...
        <div className="bg-white p-8 rounded-xl shadow-lg">
          ...
          {cart.map((item) => (
            ...
            >
              ...
              <span>₹{(item.productId.price * item.qty).toLocaleString("en-IN")}</span>
            </div>
          ))}
          ...
          <button
            ...
            className="w-full bg-orange-600 hover:bg-orange-700 text-white py-5 mt-8 rounded-lg font-bold text-xl transition"
          >
            Complete Order
          </button>
        </div>
